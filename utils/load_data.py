...
def _sent2array_ent(sent, wdict, edict, ydict, max_len):
    words = [wdict.get(x, wdict["OTHER-WORDS-ID"]) for x in sent[0]]
    ents = [edict.get(x, edict["NEGATIVE"]) for x in sent[1]]
    MAX_SEN_LEN = max_len
...
def load_data_ent(data_path, wdict, edict, ydict, max_len):
    ...
    for line in open(data_path):
        ...
        if len(line.split("\t")) < 3:
            continue
        wds = line.split("\t")[:-1]
        wds, ents = list(zip(*[x.split(" ") for x in wds]))
        ls = line.split("\t")[-1].strip().lower().split(" ")
        words, ents, labels = _sent2array_ent(
            (wds, ents, ls), wdict, edict, ydict, max_len
        )
        sen.append(words)
        ent.append(ents)
        y.append(labels)
