...
class TBNNAM:
    def __init__(self, settings, U=None):
        ...
        if U is None:
            U = np.asarray(
                rng.standard_normal(size=(settings["word_count"], settings["emb_dim"])),
                dtype=np.float32,
            )
        ...
        # entity embedding
        E = np.asarray(
            0.01 * rng.standard_normal(size=(settings["n_ent"], settings["dim_ent"])),
            dtype=np.float32,
        )
        self.ent_embedding = tf.Variable(E, name="ent_emb")

        evt_emb = np.asarray(
            0.01 * rng.standard_normal(size=(settings["n_class"], hidden_dim)),
            dtype=np.float32,
        )
        ...
        output, state = tf.nn.dynamic_rnn(
            self.encoder, self.x, sequence_length=self.lengths, dtype=tf.float32
        )

        # attention
        attention1_logits = tf.matmul(output, tf.transpose(self.x_evt, [0, 2, 1]))
        attention1_logits = (
            tf.reshape(attention1_logits, [-1, settings["max_l"]]) * self.mask
        )
        ...
        _score1 = (
            attention1_logits
            * attention1
            / tf.reduce_sum(attention1, 1, keep_dims=True)
        )
