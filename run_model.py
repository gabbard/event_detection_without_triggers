...
from utils import load_data, show_result, tools

PREFIX = os.path.join(os.getcwd(), os.path.dirname(__file__))
...
def convert2binary(data, ydict, neg_prob=0.4):
    ...
    for idx in range(len(sen)):
        for ly in ydict.values():
            ...
            if lb == 0 and random.random() > neg_prob:
                continue
    ...
    return (
        np.asarray(ret_sen, dtype="int32"),
        np.asarray(ret_ent, dtype="int32"),
        np.asarray(ret_evt, dtype="int32"),
        np.asarray(ret_label, dtype="int32"),
        np.asarray(ret_mask, dtype="float32"),
    )
...
def predict_sen(sess, sen, ent, ydict, cmodel, max_ans=3):
    ...
    for y, p in zip(labels, pred):
        ...
        if y == ydict["negative"] and p < 0.5:
            # print s
            pass
    ans = sorted(ans, key=cmp_to_key(lambda a, b: (a[1], b[1])), reverse=True)
...
def run_model(
    train_data, WORDS, settings, wdict, ydict, edict
):  # wdict, ydict used to show predicted result

    t_train_sen, t_train_ent, t_train_evt, t_train_y, t_train_mask = convert2binary(
        train_data, ydict
    )
    ...
    with tf.Session() as sess:
        ...
        while epchs < settings["n_eps"]:
            ...
            for k in range(n_batchs):
                ...
                feeddict = {
                    cmodel.sent: batch_sent,
                    cmodel.ent: batch_ent,
                    cmodel.evt: batch_evt,
                    cmodel.mask: batch_mask,
                    cmodel.y: batch_y,
                }
                ...
                print(
                    ...
                    % (epchs, (k + 1) * 100.0 / n_batchs),
                    "completed in %.2f (s)" % (time.time() - tic),
                    "loss: %.4f" % np.mean(ers),
                    end=" ",
                )
                sys.stdout.flush()
...
def train(alpha=0.25):
    ...
    train_path = "%s/data/corpus_train.txt" % PREFIX
    edict_path = "%s/data/dicts/ent_dict.txt" % PREFIX
    wdict_path = "%s/data/dicts/word_dict.txt" % PREFIX
    ydict_path = "%s/data/dicts/label_dict.txt" % PREFIX

    wdict = tools.load_dict(wdict_path)
    edict = tools.load_dict(edict_path)
    ydict = tools.load_dict(ydict_path)
    ydict = {k.lower(): v for k, v in list(ydict.items())}

    train_data = load_data.load_data_ent(train_path, wdict, edict, ydict, s["max_l"])
    word_dest_p = "%s/data/embeddings/200.txt" % PREFIX
...
def eval_model(
    test_path, model_dir, model_version
):  # wdict, ydict used to show predicted result
    ...
    test_sents, test_ents, test_y = load_data.load_data_ent(
        test_path, wdict, edict, ydict, settings["max_l"]
    )
    tf.reset_default_graph()
